pipeline {
    ...
    stages {
        ...
        stage('Quality checks') {
            steps {
                sh 'cd src; docker-compose -f docker-compose.yml -f docker-compose.jenkins.yml up mypy pylint pydocstyle pycodestyle vulture pyroma shellcheck gherkin-lint markdown-lint'
                publishHTML target: [
                    allowMissing: false,
                    alwaysLinkToLastBuild: true,
                    keepAll: false,
